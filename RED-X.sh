...
"
prompt="\007\033[00m[\033[01;32mRED-X\033[00m@\033[01;32mX -ATHEX\033[00m]$"
      ...
      if [ ! -e $bin/$tool_pac ]; then
        ...
        if [ $sudo ]; then
          ...
          if [ $? -eq 0 ]; then
            ...
            echo "\033[01;32mSorry you are offline !!\033[00m"
          fi
        else
          #check user is online or not
          wget -q --spider http://google.com
          if [ $? -eq 0 ]; then
            ...
            echo "\033[01;32mSorry \033[00m: you are \033[01;32moffline \033[00m!!\033[00m"
          fi
        fi
        # check tool is installed or not
        if [ ! -e $bin/$tool_pac ]; then
          echo "\033[01;32mSorry \033[00m: '$tool_name' \033[01;32mis not installed !!\033[00m"
        else
          echo "\033[00m'$tool_name' \033[00m: \033[01;32mis installed successfully !!\033[00m"
        fi
      else
        echo "\033[01;32mSorry \033[00m: \033[00m'$tool_name' \033[01;32mis already installed !!\033[00m"
      fi
    ...
    case $cmd in
      [1-9]|[1-9][0-9]|[1-9][0-9][0-9]|[1-9][0-9][0-9])
        if [ $cmd -le $list ]; then
          ...
          echo "\007\033[00m'$cmd': \033[01;32minvalid option!!"
          sleep 1
        fi
      ;;
      0|back|Back)
            ...
            case $tmp_cmd in
              [1-9]|[1-9][0-9]|[1-9][0-9][0-9]|[1-9][0-9][0-9])
                if [ $tmp_cmd -le $list ]; then
                  ...
                  echo "\007\033[00m'$tmp_cmd': \033[01;32minvalid option!!"
                  sleep 1
                fi
              ;;
                0|back|Back)
    ...
    case $cmd in
      1)
        ...
        if [ $? -eq 0 ]; then
          ...
          if [ -e $bin/git ]; then
            if [ $sudo ]; then
              $sudo git clone https://github.com/Athexhacker/RED-X.git ~/RED-X
              $sudo chmod +x ~/RED-X/install
              $sudo sh ~/RED-X/install
            else
              git clone https://github.com/Athexhacker/RED-X.git ~/RED-X
          ...
          echo "\007\033[01;32mRED-X updated !!\033[00m"
          sleep 1
        else
          echo "\007\033[01;32mSorry \033[00m: you are \033[01;32moffline \033[00m!!\033[00m"
          sleep 1
        fi
      ;;
      0)
...
# about RED-X function
about() {
  clear
  echo "$logo"
  echo " \033[01;32mRED-X \033[01;32mv0.1\033[00m"
  echo "RED-X is a library of all"
...
menu() {
  while true; do
    ...
    echo "$logo"
    echo " \033[00m[1] \033[01;32mAll tools \033[00m(\033[01;32m$tools tools\033[00m)"
    echo " \033[00m[2] \033[01;32mTools category"
    echo " \033[00m[3] \033[01;32mUpdate RED-X"
    ...
    case $cmd in
      ...
      ;;
      *)
        echo "\007\033[00m'$cmd': \033[01;32minvalid option!!\033[00m"
        sleep 1
      ;;
    esac
  done
...
update_process() {
  ...
  if [ -e $home/RED-X/RED-X ]; then
    if [ $sudo ]; then
      $sudo mv -v $home/RED-X/RED-X $bin
      $sudo chmod +x $bin/RED-X
    else
      mv -v $home/RED-X/RED-X $bin
      chmod +x $bin/RED-X
    fi
  fi
  if [ -d $home/RED-X ]; then
    if [ $sudo ]; then
    $sudo mv -v $home/RED-X $conf_dir
    else
      mv -v $home/RED-X $conf_dir
    fi
  fi
}
...
cli_update() {
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "\033[00m'RED-X' \033[01;32mupdated successfully !!\033[00m"
  else
    echo "\033[01;32mSorry \033[00m: you are \033[01;32moffline \033[00m!!\033[00m"
  fi
}
...
cli_install() {
  ...
  if [ $get_tool_data ]; then
    ...
    echo "\033[01;32mSorry \033[00m: '$1' \033[01;32mtool not found !!\033[00m"
    similar=`grep -i "^$1" $tools_data | cut -d"|" -f1`
    if [ "$similar" ]; then
      echo "\033[01;32mDid you mean\033[00m:"
      echo -n "\033[01;32m"
...
# remove function
cli_remove() {
  # add remove tools functionality for remove tools data now it only uninstall RED-Xx.
  # removing RED-X only
  if [ -e $bin/RED-X ]; then
    if [ -d $conf_dir/RED-X ]; then
      if [ $sudo ]; then
  ...
  # check RED-X removed or not
  if [ ! -e $bin/RED-X ]; then
    if [ ! -d $conf_dir/RED-X ]; then
      echo "\033[00m'RED-X' : \033[01;32muninstalled successfully !!\033[00m"
    fi
  fi
}
...
if [ $# -eq 1 ]; then
  case $1 in
    ...
    ;;
    help|-h)
      echo "\033[01;32mRED-X \033[01;32mv0.1"
      echo "\033[00mUsage \033[00m: \033[01;32m RED-X \033[00m[option]... [toolname]...\033[00m"
      echo "\n  \033[01;32mOption  \tDescription\033[00m"
      echo "  -----------  \t--------------------"
      ...
      echo "  update, -u  \tupdate RED-X."
      echo "  start  \tstart RED-X menu mode."
    ;;
    *)
      echo "\033[01;32mSorry \033[00m: '$1' \033[01;32minvalid option!!\033[00m"
      echo "\033[01;32mtype \033[00m:\n  'RED-X -h' or 'RED-X help' for help.\033[00m"
    ;;
  esac
elif [ $# -eq 2 ]; then
  case $1 in
    list|-l)
      case $2 in
        -a|a)
         ...
         ;;
        *)
          echo "\033[01;32mSorry \033[00m: '$2' \033[01;32minvalid option!!\033[00m"
          echo "\033[01;32mtype \033[00m:\n  'RED-X -h' or 'RED-X help' for help.\033[00m"
        ;;
      esac
    ;;
